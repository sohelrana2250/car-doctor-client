    ...
    queryFn: async () => {
      try {
        const res = await fetch(
          `https://car-doctor-server-pied-zeta.vercel.app/api/v1/dashboard_graph`,
          {
            headers: {
  ...
  return (
    <>
      ...
      <ResponsiveContainer width="100%" height={500}>
        <BarChart data={mydashboard?.data?.coursePayment}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="courseStartDate" />
          <YAxis />
          <Tooltip />
          <Legend />
          <Bar dataKey="price" fill="#73cce7" />
          <Bar dataKey="courseName" fill="#9a13d4" />
        </BarChart>
      </ResponsiveContainer>
      <h1 className=" m-3 text-4xl font-serif text-center my-3 py-2 bg-gradient-to-r from-blue-500 to-teal-400 text-white rounded-lg shadow-lg">
